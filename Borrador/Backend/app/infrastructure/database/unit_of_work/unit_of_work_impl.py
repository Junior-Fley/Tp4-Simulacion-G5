...
class SqlAlchemyUnitOfWork(IUnitOfWork):
    ...
    def __enter__(self) -> 'SqlAlchemyUnitOfWork':
        ...
        self.simu_repo = SimulacionRepositoryImpl(self.session)  # ← debe estar esto
        self.colec_repo = ColeccionRepositoryImpl(self.session)
        return self

    def __exit__(self, exc_type, exc, tb) -> None:
        try:
            if exc_type is None:
                # Solo llamar flush_buffer si el repo lo soporta
                if hasattr(self.simu_repo, 'flush_buffer'):
                    self.simu_repo.flush_buffer()
                self.session.commit()
            else:
