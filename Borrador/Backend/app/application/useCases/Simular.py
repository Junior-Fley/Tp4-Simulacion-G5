...
class Simular:
    ...
    def guardar_fila_llega_cliente(self, uow: IUnitOfWork):
        ...
        if self.hora_actual > self.hora_cierre and self.cola_equipos.cantidad() > 0:
            primer_equipo = self.cola_equipos.primero()

            # Si el equipo todavía no tiene tiempo calculado, pasar string vacío
            duracion_restante = (
                self.float_a_hora(primer_equipo.tiempo_reparacion_restante)
                if primer_equipo.tiempo_reparacion_restante is not None
                else ''
            )

            uow.simu_repo.guardar_llega_cliente_repara(
                ...
                self.tecnico.estado.value,
                round(self.rnd_reparacion, 3),
                duracion_restante,  # ← ya no explota si es None
                self.cola_clientes.cantidad(),
                self.cola_equipos.cantidad(),
                self.float_a_hora(self.tecnico.acum_atencion),
                self.float_a_hora(self.tecnico.acum_reparacion),
    ...
    def ejecutar_simulacion(self) -> int:
        ...
        # UNA SOLA sesión para toda la simulación
        with self.uow_factory() as uow:
            ...
            # Al salir del with: flush_buffer() + commit (un solo INSERT masivo)
